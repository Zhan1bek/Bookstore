...
// readFloat is a helper method on the application type that reads a string value from the URL query
// string and converts it to a float64 before returning. If no matching key is found, then it
// returns the provided default value. If the value couldn't be converted to a float64, then we
// record an error message in the provided Validator instance, and return the default value.
...
func (app *application) GetToken(w http.ResponseWriter, r *http.Request) (string, error) {
